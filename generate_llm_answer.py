...
def process_test_file(
    ...
    if type == "turf":
        prompt = (
            "Please complete the following JavaScript function for local geospatial processing, based on the function signature and the provided JSDoc-style docstring. "
            ...
            "Wrap the code with ```javascript and ``` to indicate the code block.\n\n"
...
def clean_file(model_name: str):
    ...
    raw_dirs = [raw_output_dir1]
    output_dirs = [output_dir1]
...
def clean_content(content: str) -> str:
    ...
    # 2. 提取所有代码块
    code_blocks = re.findall(r'```(?:javascript)?\s*(.*?)```', text, re.DOTALL)
...
if __name__ == "__main__":
    # System prompt
    system_prompt = (
        "You are an expert in geospatial data processing using JavaScript."
    )

    # Model configurations to test
    models_to_test=[
        {'provider': 'deepseek', 'name': 'deepseek-chat', 'name_simple': 'deepseek-chat'},
    ]
    # models_to_test = [
